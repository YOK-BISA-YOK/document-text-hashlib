...
class Document:
    ...
    def __init__(self, filename):
        ...
        self.content = None
        self.root = tk.Tk()
        self.root.config(bg='#5BC0F8') 
        self.root.title(f"Dokumen: {self.filename}")

        # Menambahkan opsi pilihan hash function
        self.hash_function = tk.StringVar(self.root)
        self.hash_function.set("MD5") # opsi default
        self.hash_options = tk.OptionMenu(self.root, self.hash_function, "SHA-256", "MD5", "SHA-1", "SHA-512", "SHA-224")
    ...
    def calculate_hash(self):
        ...
        if hash_function == "SHA-256":
            ...
            hash = hashlib.sha224(content.encode()).hexdigest()
        messagebox.showinfo(title="Hash", message=f"Hasil Hash dari dokumen : {hash}")

# Contoh penggunaan kelas
doc = Document('YokBisaYok.txt')
doc.read()
doc.root.mainloop()